...
import { ProductCardComponent } from '../directves/product-card/product-card.component';
import { NgforComponent } from "./ngfor/ngfor.component";
import { FilterComponent } from "./filter/filter.component";
import { FilterwithdropdownComponent } from "../filterwithdropdown/filterwithdropdown.component";

@Component({
  selector: 'app-directives',
  standalone: true,
  imports: [CommonModule, ProductCardComponent, NgforComponent, FilterComponent, FilterwithdropdownComponent],
  templateUrl: './directives.component.html',
  styleUrl: './directives.component.css'
})
export class DirectivesComponent {
  names:Array<string> = ["aaa","bbb","cccc","ddd","eeee"]
 ...
 products:any[]= [
  ...
  categories:string[] = Array.from(new Set(this.productlist.map(product=>product.category)))
 
  
  totalProducts:number =this.products.length
  beautyProducts:number =this.products.filter(item => item.category=='beauty').length
  groceriesProducts:number =this.products.filter(item => item.category=='groceries').length
  furnitureProducts:number =this.products.filter(item => item.category=='furniture').length

  radiodata:string = 'all'
  onFilterChanged(val:string){
    // console.log("onFilterChanged",val)
    this.radiodata = val
    if(val != 'all')
      this.productlist = this.products.filter(item => item.category==val)
    else 
  ...
  trackbyid(index:number,product:any):number{return product.id}
  }